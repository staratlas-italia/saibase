import { mintsDecimals } from '@saibase/constants';
import {
  StarAtlasNft,
  getEntityVwapPrice,
  getOrdersByType,
    ...
    const data = ships.map((ship) => {
      ...
      const buyPrice = Math.min(
        ...(orders.usdc.sell
          ...
          ?.map((o) =>
            o.price.div(new BN(10).pow(mintsDecimals.usdc)).toNumber()
          ) || [0])
      );
      ...
      const atlasBuyPrice = Math.min(
        ...(orders.atlas.sell
          .filter((order) => order.assetMint.toString() === ship.mint)
          ?.map((o) =>
            o.price.div(new BN(10).pow(mintsDecimals.atlas)).toNumber()
          ) || [0])
      );
