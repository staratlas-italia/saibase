...
/**
 * Makes a request with the `method` set to `DELETE`, the `cache` set to `no-store`, the `credentials` set to
 * `same-origin` and with the default headers applied.
 *
 * @category creators
 * @since 0.1.0
 */
export const del = pipe(appy.del, withCache('no-store'), withCredentials('same-origin'), withHeaders(defaultHeaders))
