...
describe('withCache', () => {
	...
	it('Sets the provided cache configuration to the fetch request init', () => {
		...
		return doRequest().then(() => {
			expect(fakeFetch).toHaveBeenCalledWith(endpoint, expect.objectContaining({ cache: 'no-cache' }))
		})
	})
	...
	it('Applies the last provided cache configuration if set more than once', () => {
		...
		const doRequest = pipe(request, withCache('no-cache'), withCache('default'))(endpoint)

		return doRequest().then(() => {
			expect(fakeFetch).toHaveBeenCalledWith(endpoint, expect.objectContaining({ cache: 'default' }))
		})
	})
})
